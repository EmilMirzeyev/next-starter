    ...
    const handleAdvantagesTab = () => {
        const renderAdvantagesTab = {
            [AdvantagesEnum.PARTNERSHIP]: (
                <div className="flex tablet:flex-col-reverse mobile:flex-col-reverse">
                    <div className="w-1/2 tablet:w-full tablet:mt-10 mobile:w-full mobile:px-2 mobile:pr-4 mobile:mt-8">
                        <h2 className="text-brand-700 text-36px700 tablet:text-30px700 mobile:text-30px700">
                        ...
                        <ul className="flex flex-col gap-2">
                            {[
                                ...
                                <li key={index} className="flex items-center gap-4 text-16px500 tablet:text-14px500 mobile:text-14px500">
                                    <p className="size-2.5 bg-brand-700 rounded-full flex-shrink-0"></p>
                                    <div>{text}</div>
                                </li>
                            ))}
                        </ul>
                        <Button className="rounded-[28px] mt-8 flex gap-x-2.5 items-center tablet:mt-10 tablet:w-full mobile:w-full">
                            Daxil ol
                            <ArrowRightSVG stroke="#fff" />
                        </Button>
                    </div>
                    <div className="w-1/2 tablet:w-full tablet:flex tablet:justify-center mobile:w-full">
                        <Image
                            quality={100}
                            src={AdvantagesImage}
